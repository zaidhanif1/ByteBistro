...
export class FalAITextToVideoTask extends FalAITask {
    ...
    async getResponse(response, url, headers) {
        ...
        if (typeof result === "object" &&
            ...
            throw new InferenceOutputError("Expected { video: { url: string } } result format, got instead: " + JSON.stringify(result));
        }
    }
}
export class FalAIAutomaticSpeechRecognitionTask extends FalAITask {
    ...
    async getResponse(response) {
        ...
        if (typeof res?.text !== "string") {
            throw new InferenceOutputError(`Expected { text: string } format from Fal.ai Automatic Speech Recognition, got: ${JSON.stringify(response)}`);
        }
...
export class FalAITextToSpeechTask extends FalAITask {
    ...
    async getResponse(response) {
        ...
        if (typeof res?.audio?.url !== "string") {
            throw new InferenceOutputError(`Expected { audio: { url: string } } format from Fal.ai Text-to-Speech, got: ${JSON.stringify(response)}`);
        }
        try {
            ...
            throw new InferenceOutputError(`Error fetching or processing audio from Fal.ai Text-to-Speech URL: ${res.audio.url}. ${error instanceof Error ? error.message : String(error)}`);
        }
    }
}
